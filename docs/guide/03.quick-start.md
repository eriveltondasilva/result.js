...
```typescript
result.unwrap()         // T (throws at runtime if Err)
result.unwrapOr(42)     // T (fallback value, never throws)
result.toNullable()     // T | null  (Ok → value, Err → null)
result.toValue()        // T | undefined (Ok → value, Err → undefined)
```
...
Create a Result by validating with a predicate:
...
- **[Error Handling](./04.error-handling)** — Error strategies and patterns
- **[Operation Chaining](./06.chaining)** — Advanced composition
- **[Type Safety](./08.type-safety)** — Understand TypeScript integration
