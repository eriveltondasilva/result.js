...
// Define um erro customizado para o teste de mapeamento
class CustomError extends Error {}

// TODO: remover comentários e verificar possíveis problemas em Result.fromTry
describe('Result.fromTry (Essential Tests)', () => {
  // 1. Sucesso: A função executa sem lançar exceção.
  it('should return an Ok Result when the function executes successfully', () => {
    const value = 'data'
  ...
  // 2. Falha (Padrão): A função lança um erro, e ele é capturado como Err.
  it('should return an Err Result with the original error when the function throws an error', () => {
    const originalError = new Error('Database connection failed')
    const throwingFn = () => {
      throw originalError
    }

    const result = Result.fromTry(throwingFn)

    expect(result.isErr()).toBe(true)
    expect(result.unwrapErr()).toBe(originalError)
  })

  // 3. Mapeamento: A função falha, mas o erro é transformado para um tipo específico.
  it('should map the caught error using the provided mapError function', () => {
    const rawError = 'Invalid input string'
    const mapError = (err: unknown) => new CustomError(`Failed: ${err}`)

    // Tipagem explícita para forçar a checagem do CustomError
    const result = Result.fromTry<number, CustomError>(() => {
      throw rawError
    }, mapError)

    expect(result.isErr()).toBe(true)
    expect(result.unwrapErr()).toBeInstanceOf(CustomError)
    expect(result.unwrapErr().message).toContain('Failed: Invalid input string')
  })
})
